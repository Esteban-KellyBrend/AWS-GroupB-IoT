...
const SignUp = () => {
    ...
    const signUp = (e) => {
        ...
        .then((userCredential) => { console.log(userCredential); }) // Log user credentials
    ...
    return (
        <div className="bg-gradient-to-tr to-[#431857] from-black from-30% bg-cover absolute h-screen w-screen" >
            <form className="login max-w-2xl mt-20 mx-auto flex-grow items-center justify-start text-[#C5A4D5]" onSubmit={ signUp }>
                <div className="title">
                ...
                <div className="input-box mt-11 mb-14 mx-auto w-80 border-b-4 border-[#813DAE]">
                    <input type="password" onChange={(e) => setPassword(e.target.value)}  className="w-90 border-none outline-none bg-transparent text-[#C5A4D5] text-2xl placeholder-[#C5A4D5]" placeholder="Password" />
                </div>
